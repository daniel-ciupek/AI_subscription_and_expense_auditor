...
/**
 * DeepSeek-backed categorizer. DeepSeek exposes an OpenAI-compatible chat
 * completions endpoint, so the request shape mirrors the Groq driver. We
 * still validate the JSON locally and fall back to "other" instead of
 * trusting hallucinated slugs.
 */
class DeepseekAiCategorizer implements AiCategorizerInterface
{
    private const PROMPT_VERSION = 'deepseek-chat-v1';
    ...
    public function categorize(iterable $transactions): array
    {
        ...
        try {
            ...
            Log::warning('Deepseek categorizer: connection failed', [
                'error' => $e->getMessage(),
                'count' => count($items),
            ]);
        ...
        if (! $response->successful()) {
            Log::warning('Deepseek categorizer: non-2xx response', [
        ...
        try {
            ...
            Log::warning('Deepseek categorizer: invalid JSON', ['error' => $e->getMessage()]);
    ...
    private function otherCategorization(int $transactionId, array $context): AiCategorization
    {
        return new AiCategorization(
            ...
            categorySlug: 'other',
            confidence: 0.0,
            rawResponse: array_merge(['driver' => 'deepseek', 'fallback' => true], $context),
        );
    }
}
